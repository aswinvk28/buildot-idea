...
function attachfile(){
	var path = document.getElementById('BrowserHidden').value;
	document.getElementById('showpath').innerHTML = path;

}
function attachfile1(){

	var path1 = document.getElementById('BrowserHidden1').value;
	document.getElementById('showpath1').innerHTML = path1;
		...
		<table cellpadding="0" cellspacing="0" width="97%" align="left" id="member_gen">
			...
			<tr>
				<td width="685"><div class="comment">
						<div class="comment_box_wide" style="width: 92%;">
							...
							<img src="gallery/logos/thumbs/<?php echo $companyProject['logo'];?>" width="42" class="thumbs" />
							<?php }else { ?>
							...
							<img src="gallery/photos/thumbs/<?php echo $companyProject['member_photo'];?>" width="42" class="thumbs"/>
							<?php }else {?>
			...
			<tr>
				<td colspan="2"><div class="border" style="width:96%; height:5px;">&nbsp;</div></td>
			</tr>
			<tr>
...
//$('#formbox').slideUp('fast');
					...
					<form method="post" id="projectComment" enctype="multipart/form-data">
						...
						<table cellpadding="0" cellspacing="0" border="0" width="50%" class="comment_box" style="width:480px !important;" >
							...
							<tr>     
								<td width="50px"><input type="image" value="Post" src="images/post-btn.png" style="padding-top: 5px;" width="46" height="23" /></td>
								<td width="60px"><input type="reset" value="Cancel" class="cancel_btn" /></td>
								<td width="600px"><div id="FileUpload">
										<input type="file" style="width:100px;" name="image" size="1" id="BrowserHidden" onchange="return attachfile()" />
										<div id="BrowserVisible">
											<div id="showpath" style="padding:10px 0px 0px 60px; width:400px"></div>
										</div>
									</div></td>
							</tr>
						</table>
			...
			<table width="100%">
				<tr>
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" style="width:100px; float:left;"><tr><td style="padding-top:0px;border:hidden;">
						...
						<img src="gallery/logos/thumbs/<?php echo $projectMessage['logo'] ?>" width="52"  class="thumbs"/>
						<?php }else { ?>
						<img src="gallery/logos/thumbs/default.png" width="52" height="52" class="thumbs" />
						<?php }}?></td><td style="padding-top:0px;border:hidden;">
						<?php if(!empty($projectMessage['member_photo'])) { ?>
						<img src="gallery/photos/thumbs/<?php echo $projectMessage['member_photo'] ?>" width="52" class="thumbs"/>
						<?php }else { ?>
						<img src="gallery/photos/thumbs/default.png" width="52" height="52" class="thumbs" />
						<?php }?>
                        </td></tr></table>
						</a></td>
					<td width="693"><div class="commentareatwo">
							<div class="clear"></div>
							...
							<div class="commentwrappbbox">
                            	
								<div class="comment_box_groups"> <?php echo $projectMessage['message']?>
                                <?php if($memberId == $projectMessage['member_id']) {?> <a style="float:right;" href="index.php?view=showProject&delete=1&projectId=<?php echo $projectMessage['project_id'] ?>&msg_id=<?php echo $projectMessage['msg_id']?>"><span style="float: right;"><img src="images/close.gif" /></span> </a>
								<?php } ?>
									<?php if(!empty($projectMessage['attachment'])){ ?>
									<a href="gallery/files/<?=$projectMessage['attachment']?>" target="_blank"><img src="images/icon_attach.png" width="24" style="padding:15px;" class="thumbs" /></a>
									<?php } ?>
									<br />
								</div>
								<div>
...
foreach($replies as $reply){?>
							<div class="commentarea1 replys">
								...
								<div class="messageareabox"> <a href="index.php?view=showMember&memberId=<?php echo $reply['member_id']?>"><?php echo $reply['first_name']?> <?php echo $reply['last_name']?></a>:<?php echo $reply['reply_message']?><br />
                                <?php if(!empty($reply['attachment'])){ ?>
									<a href="gallery/files/<?=$reply['attachment']?>" target="_blank"><img src="images/icon_attach.png"width="24" style="padding:5px;" class="thumbs"  /></a>
									<?php } ?>
									
									<div style="width: 100%; padding-left: 0px;float:left; font-size:8pt; color: #999; background: url(images/icons/time.png) no-repeat 70px;">
							...
							<br />
							<?php }}}else{?>
							<a href="javascript:void(0)" onclick="displayText('reply<?php echo $projectMessage['msg_id'] ?>')" class="viewcomments">View all <?php echo $replyCount?> Comments</a>
...
<table border="0" cellpadding="0" cellspacing="0" style="width:100px; float:left;"><tr><td style="padding-top:0px;border:hidden;">
                               <?php if(!empty($reply['company_id'])) { if(!empty($reply['logo'])) { ?>
									...
									<div class="messageareabox"> <a href="index.php?view=showMember&memberId=<?php echo $reply['member_id']?>"><?php echo $reply['first_name']?> <?php echo $reply['last_name']?></a>:
										<?php	echo $reply['reply_message']?><br />
										<?php if(!empty($reply['attachment'])){ ?>
										<a href="gallery/files/<?=$reply['attachment']?>" target="_blank"><img src="images/icon_attach.png" width="24" style="padding:5px;" class="thumbs"  /></a>
										<?php } ?>
									...
									<!--delete option-->
									<?php if($memberId == $reply['member_id']) {?>
									<a href="index.php?view=showProject&delete=1&projectId=<?php echo $companyProject['project_id'] ?>&reply_id=<?php echo $reply['reply_id']?>"> <span style="float: right;"><img src="<?=IMAGEURL?>icons/close.gif"></img></span> </a> 
                ...
                <div id="loading<?php echo $projectMessage['msg_id']?>"></div> 
				<div id="replytext<?php echo $projectMessage['msg_id'] ?>" style="width: 498px; background-color:#F3F3F3; border:1px solid #EBEBEB; display:none; float:left; padding:5px 15px 10px 15px;margin-top:15px;"> 
					<script type="text/javascript">
$('document').ready(function()
{
					...
					<form method="post" enctype="multipart/form-data" id="formProject<?=$co++?>" action="">
						...
						<table cellpadding="0" cellspacing="0" border="0" width="50%" class="comment_box" style="width:480px !important;">
							...
							<tr>
								...
								<td width="72%"><div id="FileUpload">
										<input type="file" style="width:100px;" name="image" size="1" id="BrowserHidden1" onchange="return attachfile1()"  />
										<div id="BrowserVisible">
											<div id="showpath1" style="padding:10px 0px 0px 80px; width:400px"></div>
										</div>
									</div></td>
							</tr>
						</table>
